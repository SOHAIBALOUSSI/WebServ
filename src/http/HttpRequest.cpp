...
// {
//     size_t startPos = _pos;
//     std::string chunkFilename = "chunkedfile" + std::to_string(time(0));
//     if (!fileCreated) {
//         std::ofstream ofile(chunkFilename, std::ios::binary);
//         if (!ofile.is_open())
//             throw 500;
//         ofile.write(reinterpret_cast<const char*>(body.data()), body.size());
//         fileCreated = true;
//         ofile.close();
//     }
//     std::ofstream ofile(chunkFilename, std::ios::app | std::ios::binary);
//     if (!ofile.is_open())
//         throw 500;
//     while (true)
//     {
...
//         body.clear();
//         body.insert(body.end(), _buffer + _pos, _buffer + _pos + chunkSize);
//         ofile.write(reinterpret_cast<const char*>(_buffer + _pos), chunkSize);
//         ofile.close();
//         _pos += chunkSize + 2;
//     }
//     return (_pos - startPos);
